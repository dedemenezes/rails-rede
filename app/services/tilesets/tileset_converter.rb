module Tilesets
  class TilesetConverter
    ...
    def initialize(tileset_params)
      ...
      @file_name = Tileset.new.replace_non_ascii_with_ascii(@tileset_params[:kml].original_filename[...-4]).downcase.gsub(
        ' ', '_'
      )
    end
    ...
    def generate_geo_json_file(file_name, geojson)
      geojson_with_icons_as_https = JSON.parse(geojson).to_h do |key, value|
        if key == 'features'
          value.to_h do |feature, f_value|
            if feature['properties']['icon'].present? && !feature['properties']['icon'].include?('https')
              feature['properties']['icon'].gsub!('http',
                                                  'https')
            end
            [feature, f_value]
          end
      ...
      geojson_with_icons_as_https['features'].select do
        _1['properties']['icon'].present?
      end.each { |f| puts f['properties']['icon'] }
      geojson_file_path = File.join(Rails.root.join('tmp', "#{file_name}.geojson"))
      File.write(geojson_file_path, geojson_with_icons_as_https.to_json)
      geojson_file_path
    end
